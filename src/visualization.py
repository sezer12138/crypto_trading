...
class Visualizer:
    ...
    def plot_equity_curve(
        self, result, title: str = "Strategy Backtest Result", save_path: str = None
    ):
        """
        绘制权益曲线和收益曲线
        ...
        ax3.fill_between(
            drawdown.index, drawdown.values, 0, color="#E74C3C", alpha=0.5, label="Drawdown"
        )
    ...
    def plot_price_with_signals(
        ...
        if buy_trades:
            ...
            ax1.scatter(
                buy_times, buy_prices, marker="^", color="green", s=100, label="Buy", zorder=5
            )
        ...
        if sell_trades:
            ...
            ax1.scatter(
                sell_times, sell_prices, marker="v", color="red", s=100, label="Sell", zorder=5
            )

        # 如果有均线，也画出来
        if "ma_short" in df.columns and "ma_long" in df.columns:
            ax1.plot(
                df.index, df["ma_short"], label="MA Short", color="orange", alpha=0.7, linewidth=1
            )
            ax1.plot(
                df.index, df["ma_long"], label="MA Long", color="purple", alpha=0.7, linewidth=1
            )
    ...
    def plot_monthly_returns(self, result, save_path: str = None):
        """绘制月度收益热力图"""
        ...
        for i in range(len(pivot_table.index)):
            for j in range(len(pivot_table.columns)):
                ...
                if not np.isnan(value):
                    text = ax.text(
                        j, i, f"{value:.1f}%", ha="center", va="center", color="black", fontsize=9
                    )
    ...
    def plot_metrics_comparison(self, results: Dict[str, object], save_path: str = None):
        """
        比较多策略的回测指标
    ...
    def create_full_report(
        self, result, df: pd.DataFrame, strategy_name: str, coin: str, output_dir: str = "results"
    ):
        """
        生成完整报告（包含所有图表）
        ...
        self.plot_equity_curve(
            result, title=f"{strategy_name} - {coin} Backtest", save_path=f"{base_name}_equity.png"
        )
        ...
        # 3. 月度收益
        self.plot_monthly_returns(result, save_path=f"{base_name}_monthly.png")
